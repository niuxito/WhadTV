				...
				$.ajax({
					...
					success: function( data ) {
						if(data.records==0) {
							...
							response(data.records, data.total, $.map( data.valueOf(self.options.root), function( item ) {
	...
	_renderHeader: function(ul, colModel ) {
		...
		// Check if column is ordered or not to provide asc/desc icon
			if(col.columnName==self.cssCol){
				div.append('<div class="cg-colHeader" style="width:'+col.width+'%;'+hide+' text-align:'+col.align+'"><label class="cg-colHeader-label" id="'+ col.columnName +'">'
